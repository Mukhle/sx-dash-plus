	...
	for (let key in allPunishments) {
		if(allPunishments.hasOwnProperty(key)) {
			if(allPunishments[key].length > 0) {
				...
				a.addEventListener('click', clickEventListener.bind(null, key, "Altid", modalTitle, modalTable, allPunishments));
	...
	for (let key in monthPunishments) {
		if(monthPunishments.hasOwnProperty(key)) {
			if(monthPunishments[key].length > 0) {
				...
				a.addEventListener('click', clickEventListener.bind(null, key, "Seneste måned", modalTitle, modalTable, monthPunishments));
	...
	for (let key in weekPunishments) {
		if(weekPunishments.hasOwnProperty(key)) {
			if(weekPunishments[key].length > 0) {
				...
				a.addEventListener('click', clickEventListener.bind(null, key, "Seneste uge", modalTitle, modalTable, weekPunishments));
	...
	for (let key in dayPunishments) {
		if(dayPunishments.hasOwnProperty(key)) {
			if(dayPunishments[key].length > 0) {
				...
				a.addEventListener('click', clickEventListener.bind(null, key, "Seneste døgn", modalTitle, modalTable, dayPunishments));
