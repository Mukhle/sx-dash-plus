...
export async function execute(): Promise<void> {
	const [buttongroup, steamidParts] = getLookupContainerFromHeaderText<[HTMLDivElement, RegExpMatchArray], [null, null]>(
		'Spillerinformation',
		(element) => {
			const group = element.parentElement?.parentElement?.querySelector<HTMLDivElement>('div.btn-group')
	...
	if (buttongroup === null || steamidParts === null) return
